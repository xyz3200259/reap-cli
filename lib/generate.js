...
function renderTeamplate(template, data, target, append) {
  ejs.renderFile(template, data, {}, (err, str) => {
    if (err) {
      console.log(chalk.red(`render template  ${template} error happen`, err));
    } else {
    ...
    .then(function (answers) {
      ...
      generateUnderTargetPath((target) => {
        ...
        try {
          ...
          Object.assign(metadata,{_code:code,_name:name})
          renderTeamplate(path.join(__dirname,'../boilerplate/functions.ejs'), {functions}, `${target}/functions.json`);

          renderTeamplate(path.join(__dirname,'../boilerplate/crud/frontend/model.ejs'), {metadata}, `${target}/models/${name}.js`);
          console.log(chalk.green(`generated model success：${target}/models/${name}.js`));
